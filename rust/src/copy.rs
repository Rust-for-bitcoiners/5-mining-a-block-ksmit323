...
fn create_coinbase_transaction_2(reward: u64, address: &str) -> BitcoinTransaction {
    let mut coinbase_input = TxIn::default();
    coinbase_input.previous_output = OutPoint::null(); // Coinbase has no prev output

    // Coinbase input must have a script_sig of 2-100 bytes
    // Gonna use block height (4 bytes) + some padding
    let block_height: i32 = 1000000; // would need to replace with actual block height; don't care rn
    let mut script_sig = Vec::new();
    script_sig.extend_from_slice(&block_height.to_le_bytes());
    script_sig.extend_from_slice(b"Billy Idol"); // Any data is fine
    coinbase_input.script_sig = Script::from(&script_sig);
}
